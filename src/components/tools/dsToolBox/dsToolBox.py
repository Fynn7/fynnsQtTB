...
class DSToolBox(BaseWindow):
    ...
    def setupMenubar(self):
        ...
        toCSV_action.triggered.connect(
            lambda: self.handle_file_converter("csv"))

        toExcel_action = QAction("To Excel", self)
        toExcel_action.triggered.connect(
            lambda: self.handle_file_converter("excel"))
        toExcel_action.setEnabled(False)

        toJSON_action = QAction("To JSON", self)
        toJSON_action.triggered.connect(
            lambda: self.handle_file_converter("json"))
