...
class CustomDict(BaseWindow):
    ...
    def setup_ui(self):
        self.addWidgetToLayout("QLabel","Choose a table from the selected file:")
        self.addWidgetToLayout("QPushButton","Select Table",self.select_column)
        self.mapping_name_input = self.addWidgetToLayout("QLineEdit","Mapping 1")
        self.mapping_name_input.setPlaceholderText("Enter a name for the mapping")
        ...
        self.custom_dict_storage_scrollArea.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOn)
        self.custom_dict_storage_label = QLabel("Saved Dictionary/Mapping:\n\n")
        self.custom_dict_storage_label.setWordWrap(True)
        self.custom_dict_storage_scrollArea.setWidget(self.custom_dict_storage_label)
    ...
    def setup_menubar(self):
        ...
        clear_storage_action=QAction("Clear All Permanent Mapping Storage",self)
    ...
    def select_column(self) -> None:
        ...
        if not self.mapping_name_input.text():
            QMessageBox.critical(self, "No Mapping Name", "Please enter a name for the mapping.")
            return
        file_path = _select_file("Excel Files (*.xlsx *.xls *.csv);; CSV Files (*.csv)")
        if not file_path:
            return
        ...
        choose_column_dialog = ChooseColumn(got_chosen_table.columns,only_choose_one=True,description="Choose the keys of the dictionary:")
        if choose_column_dialog.exec()==QDialog.DialogCode.Accepted:
            got_chosen_keys_column:str = choose_column_dialog.get_chosen_columns()[0]
            print("CustomDict().select_column() got chosen keys column:", got_chosen_keys_column)
        else:
            # user canceled the selection
            return
        ...
        choose_column_dialog = ChooseColumn(got_chosen_table.columns,only_choose_one=False,description="Choose the values of the dictionary:")
        if choose_column_dialog.exec()==QDialog.DialogCode.Accepted:
            got_chosen_values_columns:list= choose_column_dialog.get_chosen_columns()
            print("CustomDict().select_column() got chosen values column:", got_chosen_values_columns)
        else:
            # user canceled the selection
            return
        ...
        self.data[mapping_name] = dict(zip(got_chosen_table[got_chosen_keys_column],got_chosen_table[got_chosen_values_columns].values.tolist()))
    ...
    def clear_storage(self):
        ...
        reply = QMessageBox.question(self, "Clear Storage", "Do you really want to clear all the mapping storage?", QMessageBox.Yes | QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.update_data_file({
                "custom_dicts":{}
            })
            print("CustomDict().clear_storage() cleared storage.")
            self.custom_dict_storage_label.setText("Saved Dictionary/Mapping:\n\n")
