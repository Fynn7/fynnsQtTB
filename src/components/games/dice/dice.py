...
class Dice(BaseWindow):
    def __init__(self):
        ...
        self.WINDOW_TITLE = "Dice Game"
        super(Dice, self).__init__()
        self.setup_ui()
        self.setup_menubar()

        # override the QVBoxlayout to a Grid layout
        self.updateLayout(QGridLayout())
    ...
    def setup_menubar(self):
        ...
        game_settings_dialog = QAction("Game Settings", self)
    ...
    def reset_dice_labels(self):
        ...
        self.addWidgetToLayout(
            "QLabel", text="Amount of dices: "+str(self.amount_of_dice))
        self.addWidgetToLayout(
            "QLabel", text="Dimension of dices: "+str(self.dim_of_dice))

        # dice labels
        amount = self.amount_of_dice
        for i in range(1, amount+1):
            exec(
                f'self.dice_label_{i}=self.addWidgetToLayout("QLabel", text="⚀")')
            print(f'self.dice_label_{i}', 'generated')

        # roll button
        self.addWidgetToLayout(
            "QPushButton", text="Roll Dice", clickedConn=self.roll_dice)
