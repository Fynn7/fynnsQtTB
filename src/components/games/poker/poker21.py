...
class Poker21(BaseWindow):
    ...
    def setup_ui(self) -> None:
        ...
        self.clear_bet_button.clicked.connect(
            lambda: self.bet(-self.bet_amount))
    ...
    def hit(self) -> None:
        ...
        if self.player_score > 21:
            self.info_label.setText(
                f"Sorry, you busted! You lost {self.bet_amount}€.")
            self.cash_out(False)
    ...
    def stand(self) -> None:
        ...
        if self.dealer_score > 21:
            self.info_label.setText(
                f"Dealer busts! You win {self.bet_amount}€.")
            self.cash_out(True)
        elif self.player_score > self.dealer_score:
            if self.player_score == 21 and len(self.player_hand) == 2:
                self.info_label.setText(
                    f"Blackjack! Player wins x2 bet: {self.bet_amount*2}€.")
            ...
            self.info_label.setText(
                f"Dealer wins. You lost {self.bet_amount}€.")
            self.cash_out(False)
        else:
    ...
    def cash_out(self, won: bool, blackjack: bool = False) -> None:
        ...
        self.getCurrentMenubar().actions()[0].setText(
    ...
    def update_player_cards_ui(self) -> None:
        self.player_cards_label.setText(
            "\t".join([f"{card[0]}{card[1]}" for card in self.player_hand]))

    def update_dealer_cards_ui(self) -> None:
        # only shows dealer's first card, the rest are hidden as "?" shown
        self.dealer_cards_label.setText(
            f"{self.dealer_hand[0][0]}{self.dealer_hand[0][1]}"+("\t?")*(len(self.dealer_hand)-1))

    def show_dealer_cards_ui(self) -> None:
        self.dealer_cards_label.setText(
            "\t".join([f"{card[0]}{card[1]}" for card in self.dealer_hand]))
