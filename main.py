...
class ToolBoxUI(BaseWindow):
    ...
    def setupMenubar(self)->None:
        ...
        pomodoro_timer_action.triggered.connect(lambda :self.open_component_window("Tools","PomodoroTimer"))
        ...
        word_simulator_action.triggered.connect(lambda :self.open_component_window("Tools","WordSimulator"))
        components_menu.addAction(word_simulator_action)

        # ml toolbox action
        ml_toolbox_action=QtGui.QAction("ml toolbox",self)
        ml_toolbox_action.triggered.connect(lambda :self.open_component_window("Tools","MlToolBox"))
    ...
    def open_component_window(self, class_name:str,component_name: str) -> None:
        ...
        if self.components[class_name][component_name]:
            self.showMessageBox(msgType="warning",msg=f"{class_name}.{component_name} 已经打开")
        else:
